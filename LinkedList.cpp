...
void LinkedList::split(LinkedList& list1, LinkedList& list2)
{
	...
	// creating two sublists using set method
...
{
	/***** Complete this member function. *****/
	tail->next = other_list.get_head(); // select head
	tail = other_list.get_tail(); //update tail
	size += other_list.get_size(); //increament size
	other_list.reset();
}
